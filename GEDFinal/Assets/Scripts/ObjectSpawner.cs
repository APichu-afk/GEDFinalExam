        ...
        if (initialSpawn.transform.childCount < limit)
        {
            ...
            if (InGameTimer <= 0f)
            {
                for (int i = 0; i < amount; i++)
                {
                    ...
                    Instantiate(SpawningTheCubes, new Vector2(this.transform.position.x + RandomNumber(), this.transform.position.y + RandomNumber()), Quaternion.identity, initialSpawn.transform);
                }
